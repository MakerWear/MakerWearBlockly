...
var yargs = require('yargs').usage('Usage: electron-rebuild --version [version] --module-dir [path]').help('h').alias('h', 'help').describe('v', 'The version of Electron to build against').alias('v', 'version').describe('n', 'The NODE_MODULE_VERSION to compare against (process.versions.modules)').alias('n', 'node-module-version').describe('f', 'Force rebuilding modules, even if we would skip it otherwise').alias('f', 'force').describe('a', "Override the target architecture to something other than your system's").alias('a', 'arch').describe('m', 'The path to the node_modules directory to rebuild').alias('m', 'module-dir').describe('w', 'A specific module to build, or comma separated list of modules').alias('w', 'which-module').describe('e', 'The path to electron-prebuilt').alias('e', 'electron-prebuilt-dir').describe('p', 'Enable the ugly (and hopefully not needed soon enough) node-pre-gyp path fixer').alias('p', 'pre-gyp-fix').describe('c', 'The npm command to run').alias('c', 'command').describe('d', 'Custom header tarball URL').alias('d', 'dist-url').describe('id', 'Ignore devDependencies').alias('id', 'ignore-devdeps').describe('io', 'Ignore optionalDependencies').alias('io', 'ignore-optdeps').describe('l', 'Log the rebuild process').alias('l', 'log').epilog('Copyright 2015');
...
if (!argv.e) {
  argv.e = (0, _electronLocater.locateElectronPrebuilt)();
} else {
...
shouldRebuildPromise.then(function (x) {
  ...
  return (0, _mainJs.installNodeHeaders)(argv.v, argv.d, null, argv.a).then(function () {
    return (0, _mainJs.rebuildNativeModules)(argv.v, argv.m, argv.w, null, argv.a, argv.c, argv.id, argv.io, argv.log);
  }).then(function () {

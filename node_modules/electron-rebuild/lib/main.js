...
var checkForInstalledHeaders = function checkForInstalledHeaders(nodeVersion, headersDir) {
  var canaryNode, canaryIoJS;
  return _regeneratorRuntime.async(function checkForInstalledHeaders$(context$1$0) {
    while (1) switch (context$1$0.prev = context$1$0.next) {
      case 0:
        canaryNode = _path2['default'].join(headersDir, '.node-gyp', nodeVersion, 'common.gypi');
        canaryIoJS = _path2['default'].join(headersDir, '.node-gyp', 'iojs-' + nodeVersion, 'common.gypi');

        if (fs.existsSync(canaryNode) || fs.existsSync(canaryIoJS)) {
          context$1$0.next = 4;
          break;
        }
...
var spawnWithHeadersDir = function spawnWithHeadersDir(cmd, args, headersDir, cwd) {
  var verbose = arguments.length <= 4 || arguments[4] === undefined ? false : arguments[4];
  var env, opts;
  return _regeneratorRuntime.async(function spawnWithHeadersDir$(context$1$0) {
    while (1) switch (context$1$0.prev = context$1$0.next) {
      case 0:
        ...
        context$1$0.next = 7;
        return _regeneratorRuntime.awrap((0, _spawn2['default'])({ cmd: cmd, args: args, opts: opts }, verbose));
...
function rebuildNativeModules(nodeVersion, nodeModulesPath) {
  ...
  var ignoreDevDeps = arguments.length <= 6 || arguments[6] === undefined ? false : arguments[6];
  var ignoreOptDeps = arguments.length <= 7 || arguments[7] === undefined ? false : arguments[7];
  var verbose = arguments.length <= 8 || arguments[8] === undefined ? false : arguments[8];
  var cmd, args, packageJSON, modules;
    ...
    while (1) switch (context$1$0.prev = context$1$0.next) {
      ...
      case 3:
        ...
        if (ignoreDevDeps || ignoreOptDeps) {
          ...
          packageJSON = require(_path2['default'].resolve(nodeModulesPath, '..', 'package.json'));
          modules = _Object$keys(packageJSON.dependencies);

          if (!ignoreDevDeps) {
            modules = modules.concat(_Object$keys(packageJSON.devDependencies));
          }
          if (!ignoreOptDeps) {
            modules = modules.concat(_Object$keys(packageJSON.optionalDependencies));
          }
          args = args.concat(modules);
        }

        args.push('--runtime=electron', '--target=' + nodeVersion, '--arch=' + (arch || process.arch), '--update-binary');

        context$1$0.next = 10;
        return _regeneratorRuntime.awrap(spawnWithHeadersDir(cmd, args, headersDir, nodeModulesPath, verbose));
